...
typedef struct {
	...
	/*name*/
	char name[256];
	/*ptr previously allocated using kmalloc*/
	char* ptr;
	/*Kernel timer*/
	struct timer_list my_timer;
	/*Milliseconds to wait for the timer*/
	unsigned long to_wait;
	/*Size to alloc*/
	size_t to_alloc;
}myfile;
/*Inits a my_file object*/
int myfile_init(myfile*,struct file_operations*,unsigned int,unsigned int,char*,unsigned long);
